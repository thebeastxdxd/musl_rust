...
# Build musl 
RUN curl -L http://www.musl-libc.org/releases/musl-$MUSL_VER.tar.gz | tar zxf  - && \
    cd musl-$MUSL_VER && \
    ./configure && \
    make -j$(nproc) && make install
...
RUN wget "https://static.rust-lang.org/rustup/archive/${RUSTUP_VER}/${RUST_ARCH}/rustup-init" && \
    chmod +x rustup-init && \
...
WORKDIR /app
COPY . .
RUN --mount=type=cache,target=/root/.cargo/registry cargo +nightly build -Z build-std=std,panic_abort -Z build-std-features=panic_immediate_abort --target x86_64-unknown-linux-musl --release
